...
const { loginDirectHttp, loginAndGetJar } = require("./connector/portalSession");
...
async function runEngine() {
  ...
  try {
    ...
    try {
      // Login diretto HTTP (~200ms) con fallback automatico a Puppeteer
      console.log("Avvio login diretto HTTP per candidato", candidate.id);
      let jar;
      try {
        jar = await loginDirectHttp({
        ...
        console.warn("Login diretto fallito, uso Puppeteer:", loginHttpErr.message);
